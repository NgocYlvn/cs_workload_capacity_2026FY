...
def chart_resolution(df: pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=resolved_x, y=agg["Resolution Rate"], name="CS Resolution Rate", mode="lines+markers+text", line=dict(color="#F97316", width=3), marker=dict(size=8, color="#F97316", line=dict(color="#FFFFFF", width=1.5)), text=agg["Resolution Rate"], texttemplate="%{text:.1%}", textposition="bottom center", textfont=dict(color="#C2410C", size=11), yaxis="y2", customdata=agg["Month"], hovertemplate="%{customdata}<br>CS Resolution Rate: %{y:.1%}<extra></extra>"))
